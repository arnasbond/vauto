 ...
 * Keep in sync with server/src/shared/vision-object-labels.ts
 */
...
export function looksLikeEnglishVisionLabel(label: string): boolean {
  ...
  // Common LT marketplace tokens without diacritics — keep as-is.
  if (
...
export function localizeVisionObjectLabel(raw: string): string {
  ...
  // Already LT or unknown proper noun — keep.
  if (!looksLikeEnglishVisionLabel(noun)) {
    return trimmed;
  }

  // Soft fallback: keep text but prefer mapped phrases inside compound labels.
  let next = noun;
