...
import type { PlannerDecision } from "../planner/planner-types.js";
...
describe("R4.3C — listing continuity (KEEP / UPDATE)", () => {
  ...
  it("end-to-end: non-vehicle draft preserves facts across conversational enrichment and correction turns", async () => {
    ...
    const buildDecision = (args?: { attributes?: Record<string, string> }): PlannerDecision => ({
      intent: "sell_update",
      goal: "update draft",
    ...
    try {
      ...
      const d1 = res1.actions.type === "listing_draft" ? res1.actions.listingDraft : null;
      assert.ok(d1, "listing draft side effect present");
      ...
      const d2 = res2.actions.type === "listing_draft" ? res2.actions.listingDraft : null;
      assert.ok(d2, "listing draft side effect present");
  ...
  it("cross-vertical isolation: non-vehicle drafts do not receive vehicle specs", async () => {
    ...
    if (res.actions?.type === "listing_draft") {
      const d = res.actions.listingDraft;
