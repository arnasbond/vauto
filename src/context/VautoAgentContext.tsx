  ...
  const sendAgentMessage = useCallback(
    async (
      ...
      try {
        ...
        if (threadLink?.anonSessionToken && threadLink.threadId && isGenuinelyAuthenticated) {
          ...
          if (claimed.ok) {
            ...
            // Caller is not authenticated on the server side — preserve the
            // anonymous thread link and token so anonymous continuation proceeds cleanly.
            persistAgentThreadLink({
              threadId: threadLink.threadId,
              version: threadLink.version,
              anonSessionToken: threadLink.anonSessionToken,
            });
          } else {
