...
export function PrivacySettingsCard() {
  const { gdprConsent, acceptGdprConsent, revokeGdprConsent } = useVauto();
  ...
  return (
    <div className="vauto-dashboard-card rounded-2xl p-4">
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl bg-black/20 px-3 py-2.5">
        ...
        <button
          type="button"
          onClick={() => (gdprConsent ? revokeGdprConsent() : acceptGdprConsent())}
      ...
      <p className="mt-2 text-[10px] text-slate-500">
        Būsena: {gdprConsent ? "Sutikta" : "Nesutikta — mikrofonas/kamera prašys sutikimo"}
      </p>
    </div>
  );
}
