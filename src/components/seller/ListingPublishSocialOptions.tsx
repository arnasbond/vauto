...
export function ListingPublishSocialOptions({ className }: { className?: string }) {
  ...
  return (
    <section
      className={cn(
        "rounded-2xl border border-[var(--vauto-border,#e5e7eb)] bg-[color-mix(in_srgb,var(--vauto-text-main,#111)_4%,transparent)] p-4",
        className
      )}
    >
...
function SocialCheckbox({
  ...
  return (
    <label className="flex cursor-pointer items-start gap-3 rounded-xl bg-[var(--vauto-card-bg,#fff)] px-3 py-2.5 ring-1 ring-[var(--vauto-border,#e5e7eb)]">
      <input
        type="checkbox"
        checked={checked}
      ...
      />
      <span className="text-xs leading-snug text-[var(--vauto-text-main,#374151)]">
        {label}
      </span>
    </label>
  );
