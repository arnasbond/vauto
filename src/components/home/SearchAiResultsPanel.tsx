...
export function SearchAiResultsPanel({ onFollowUp }: SearchAiResultsPanelProps) {
  ...
  const followUpSubmitRef = useRef(false);
  ...
  useEffect(() => {
    ...
    followUpSubmitRef.current = false;
  ...
  const handleFollowUp = (e: React.FormEvent) => {
    ...
    followUpSubmitRef.current = true;

    setFollowUpMessages((prev) => [...prev, q]);
    ...
    >
      ...
      {followUpMessages.length > 0 && (
        ...
        <div className="mt-5 space-y-2">
          ...
          {followUpMessages.map((text, i) => (

            <div

              key={`follow-up-${i}-${text.slice(0, 24)}`}

              className="ml-auto max-w-[95%] rounded-2xl rounded-tr-md bg-[var(--vauto-primary)] px-3.5 py-2.5 text-sm leading-relaxed text-[var(--vauto-primary-contrast)]"
      ...
      <form onSubmit={handleFollowUp} className="mt-4">
        ...
        <div className="home-ai-followup-shell flex items-center gap-2 rounded-2xl border py-1.5 pl-3.5 pr-1.5 shadow-sm">
          ...
          <button
            ...
            disabled={followUpDraft.trim().length < 3 || searchLoading || insightLoading}
